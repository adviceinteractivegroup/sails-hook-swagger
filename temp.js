...
function prettifyObject(memoryUsage) {
  memoryUsage.residentSetSize = memoryUsage.rss + ' B (' +
                                prettyBytes(memoryUsage.rss) + ')';
  delete memoryUsage.rss;
  memoryUsage.heapTotal = memoryUsage.heapTotal + ' B (' +
                          prettyBytes(memoryUsage.heapTotal) + ')';
  memoryUsage.heapUsed = memoryUsage.heapUsed + ' B (' +
                         prettyBytes(memoryUsage.heapUsed) + ')';
  return memoryUsage;
};
    ...
    defaults: {
      dev: {
        ...
        requestLogger: {
          ...
          onBegin: false, // If set, `logRequests` will log the end of every reqeust (even in production!)
          // If set to 'never', it will not log regardless of the environment
          onEnd: true
        }
      }
    ...
    routes: {
      before: {
        ...
        '/*': function(req, res, next) {
          ...
          if (!isObject(sails.config.dev) ||
              !isObject(sails.config.dev.requestLogger)) {
            sails.config.dev.requestLogger = {
              onBegin: false, onBeginDev: true, onEnd: true, onEndDev: true
            }
          }
          ...
          if ((process.env.NODE_ENV !== 'production' &&
            ...
            // Default logger
              ...
              sails.log.verbose(' -> ' + req.method.toUpperCase() + ' ' +
                                req.path + '');
            }
          }
          ...
          if ((process.env.NODE_ENV !== 'production' &&
            ...
            res.once('finish', function() {
              ...
              // Default logger
                ...
                sails.log.verbose(' -> ' + metadata.method.toUpperCase() + ' ' +
                                  metadata.path + ' \x1b[' + color + 'm' +
                                  res.statusCode + '\x1b[0m   ( -> ' +
                                  metadata.responseTime + 'ms, ' +
                                  prettyBytes(parseInt(res._headers['content-length'],
                                    10)) + ')');
              }
            });
          }
        ...
        'get /dev': function(req, res) {
          if (process.env.NODE_ENV === 'production' &&
              (!isObject(sails.config.dev) || !sails.config.dev.enabled)) {
            return res.notFound();
          }
          return res.send('' + '' +
                          ...
                          '<br/>' + '<h1>Runtime reference info</h1>' +
                          '<br/>' + '<em>development only</em>' + '<br/>' +
                          '<br/>' + '<br/>' +
                          '<a href="/dev/routes">See all routes</a>' + '<br/>' +
                          '<a href="/dev/session">See current user session</a>' +
                          '<br/>' +
                          '<a href="/dev/memory">See current memory usage</a>' +
                          '<br/>' +
                          '<a href="/dev/dependencies">See actual versions of node_module dependencies</a>' +
                          '<br/>' +
                          '<a href="/dev/config">See whole Sails configuration</a>' +
                          '<br/>' +
                          '<a href="/dev/env">See loaded Evnironment variables</a>' +
                          '<br/>' + '');
        },
        ...
        'put /dev/gc': function(req, res) {
          ...
          return res.json({
            Before: prettifyObject(before),
            After: prettifyObject(after),
            Diff: prettifyObject(diff)
          });
        },
        ...
        'get /dev/dependencies': function(req, res) {
          var dependencies = fsx.readJsonSync(path.resolve(sails.config.appPath,
            'package.json')).dependencies;
          return res.json(reduce(dependencies,
            function(memo, semverRange, depName) {
              var actualDependencyVersion = fsx.readJsonSync(path.resolve(sails.config.appPath,
                path.join('node_modules', depName, 'package.json'))).version;

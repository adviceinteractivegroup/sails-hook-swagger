  ...
  return {
    ...
    defaults: {
      ...
      blueprints: {
        ...
        actions: true, // e.g. '/frog': 'FrogController.index'
        index: true, // e.g. '/frog/find/:id?': 'FrogController.find'
        shortcuts: true, // e.g. 'get /frog/:id?': 'FrogController.find'
        rest: true,
    ...
    bindShadowRoutes: function() {
        ...
        function eachController(controller, controllerId) {
          ...
          if (config.prefix) {
            if (!_(config.prefix).isString()) {
              sails.after('lifted', function() {
                sails.log.blank();
                sails.log.warn(util.format('Ignoring invalid blueprint prefix configured for controller `%s`.',
                  globalId));
                sails.log.warn('`prefix` should be a string, e.g. "/api/v1".');
                STRINGFILE.logMoreInfoLink(STRINGFILE.get('links.docs.config.blueprints'),
                  sails.log.warn);
              });
              return;
            }
            if (!config.prefix.match(/^\//)) {
              ...
              sails.after('lifted', function() {
                sails.log.blank();
                sails.log.warn(util.format('Invalid blueprint prefix ("%s") configured for controller `%s` (should start with a `/`).',
                  originalPrefix, globalId));
                sails.log.warn(util.format('For now, assuming you meant:  "%s".',
                  config.prefix));
                STRINGFILE.logMoreInfoLink(STRINGFILE.get('links.docs.config.blueprints'),
                  sails.log.warn);
              });
          ...
          if (config.restPrefix) {
            if (!_(config.restPrefix).isString()) {
              sails.after('lifted', function() {
                sails.log.blank();
                sails.log.warn(util.format('Ignoring invalid blueprint rest prefix configured for controller `%s`.',
                  globalId));
            ...
            if (!config.restPrefix.match(/^\//)) {
              ...
              sails.after('lifted', function() {
                sails.log.blank();
                sails.log.warn(util.format('Invalid blueprint restPrefix ("%s") configured for controller `%s` (should start with a `/`).',
                  originalRestPrefix, globalId));
                sails.log.warn(util.format('For now, assuming you meant:  "%s".',
                  config.restPrefix));
                STRINGFILE.logMoreInfoLink(STRINGFILE.get('links.docs.config.blueprints'),
                  sails.log.warn);
          ...
          _.each(actions, function eachActionID(actionId) {
            ...
            var opts = _.merge({
              action: actionId, controller: controllerId
            }, routeOpts);
            ...
            if (config.actions) {
              var actionRoute = baseRoute + '/' + actionId.toLowerCase() +
                                '/:id?';
              sails.log.silly('Binding action (' + actionId.toLowerCase() +
                              ') blueprint/shadow route for controller:',
                controllerId);
              sails.router.bind(actionRoute, controller[actionId.toLowerCase()],
                null, opts);
              if ([
                    'sails', 'identity', 'globalid'
                  ].indexOf(actionId.toLowerCase()) === -1) {
            ...
            if (config.index !== false && actionId.match(/^index$/i)) {
              sails.log.silly('Binding index blueprint/shadow route for controller:',
                controllerId);
          ...
          var modelId = config.model || routeConfig.model ||
                        (modelFromGlobalId && modelFromGlobalId.identity) ||
                        controllerId;
          ...
          if (sails.hooks.orm && sails.models && sails.models[modelId]) {
            ...
            var _getAction = _.partial(_getMiddlewareForShadowRoute,
              controllerId);
            ...
            routeOpts = _.merge({associations: _.cloneDeep(Model.associations)},
              routeOpts);
            ...
            var _bindRoute = function(path, action, options, type) {
              ...
              options = _.extend({}, options,
                {action: action, controller: controllerId});
            ...
            if (config.shortcuts) {
              ...
              sails.log.silly('Binding shortcut blueprint/shadow routes for model ',
                modelId, ' on controller:', controllerId);

              _bindRoute(_getRoute('%s/find'), 'find', undefined, 'shortcuts');
              _bindRoute(_getRoute('%s/find/:id'), 'findOne', undefined,
                'shortcuts');
              _bindRoute(_getRoute('%s/create'), 'create', undefined,
                'shortcuts');
              _bindRoute(_getRoute('%s/update/:id'), 'update', undefined,
                'shortcuts');
              _bindRoute(_getRoute('%s/destroy/:id?'), 'destroy', undefined,
                'shortcuts');
              ...
              _(Model.associations).where({type: 'collection'}).forEach(function(association) {
                ...
                var _getAssocRoute = _.partialRight(util.format, baseRoute,
                  alias);
                var opts = _.merge({alias: alias}, routeOpts);

                sails.log.silly('Binding "shortcuts" to association blueprint `' +
                                alias + '` for', controllerId);
                _bindRoute(_getAssocRoute('%s/:parentid/%s/add/:id?'), 'add',
                  opts, 'shortcuts');
                _bindRoute(_getAssocRoute('%s/:parentid/%s/remove/:id?'),
                  'remove', opts, 'shortcuts');
              });
            }

            // Bind "rest" blueprint/shadow routes
            if (config.rest) {
              sails.log.silly('Binding RESTful blueprint/shadow routes for model+controller:',
                controllerId);

              _bindRoute(_getRestRoute('get %s'), 'find', undefined, 'rest');
              _bindRoute(_getRestRoute('get %s/:id'), 'findOne', undefined,
                'rest');
              _bindRoute(_getRestRoute('post %s'), 'create', undefined, 'rest');
              _bindRoute(_getRestRoute('put %s/:id'), 'update', undefined,
                'rest');
              _bindRoute(_getRestRoute('post %s/:id'), 'update', undefined,
                'rest');
              _bindRoute(_getRestRoute('delete %s/:id?'), 'destroy', undefined,
                'rest');
              ...
              _(Model.associations).where({type: 'collection'}).forEach(function(association) {
                var alias = association.alias;
                var _getAssocRoute = _.partialRight(util.format, baseRestRoute,
                  alias);
                var opts = _.merge({alias: alias}, routeOpts);
                sails.log.silly('Binding RESTful association blueprint `' +
                                alias + '` for', controllerId);

                _bindRoute(_getAssocRoute('post %s/:parentid/%s/:id?'), 'add',
                  opts, 'rest');
                _bindRoute(_getAssocRoute('delete %s/:parentid/%s/:id?'),
                  'remove', opts, 'rest');
              });
              ...
              _(Model.associations).forEach(function(association) {
                ...
                _bindRoute(_getAssocRoute('get %s/:parentid/%s/:id?'),
                  'populate', opts, 'rest');
              });
            }
          }
      ...
      function _getMiddlewareForShadowRoute(controllerId, blueprintId) {
        ...
        return sails.middleware.controllers[controllerId][blueprintId.toLowerCase()] ||
               hook.middleware[blueprintId];
      }
    }
